...
const Lang = function() {
  ...
  this.start = (run) =>{
    ...
    if (process.argv[2]!=null) {
      ...
      fs.readFile("index.now", (err, data) => {
        if (err) {
          console.error(err);
          return;
        }
        code = data.toString();
        const out = run(code);
        if (process.argv[2]=='--getjsandrun') {
          fs.writeFile("index.now"
			  .replace('.now', '.js'), out,
          function(err) {
            if (err) throw err;
            eval(out);
					  });
        } else if (process.argv[2]=='--getjs') {
          fs.writeFile("index.now".replace('.now', '.js'), out,
			   function(err) {
            if (err) throw err;
					  });
        } else {
    ...
    document.addEventListener("DOMContentLoaded", function(event) { 
      ...
      d.innerHTML = run(document.querySelector("nowscript").innerHTML);
      document.querySelector("nowscript").innerHTML = "";
  ...
  this.part = (code) => {
    ...
    let cls = 0;
    for (elm in csplit) {
      if (elm == (csplit.length-1) || (csplit[elm] == ';' && !infun && !instr)) {
        if(!(csplit[elm] == ';' && !infun && !instr)) elm++;
